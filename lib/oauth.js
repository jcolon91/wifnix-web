...
/* Nada de esto se lee al cargar el modulo, y el motivo es concreto:
   en server.js el require de este archivo esta en la linea 26 y el
   dotenv.config() en la 32. Un modulo que leyera process.env al
   cargarse veria el entorno todavia vacio. Con el secreto eso reventaba
   el arranque entero del API; con BASE y PORTAL era peor, porque no
   fallaban: se quedaban con el valor por defecto y se tragaban en
   silencio lo que dijera el .env.

   Leyendolo al usarse, el orden de los require deja de importar. */
...
function BASE() {
  return process.env.API_URL || 'https://api.wifnix.com';
}

function PORTAL() {
  return process.env.PORTAL_URL || 'https://portal.wifnix.com';
}
...
// Se deriva la primera vez que se usa, no al cargar el modulo: para
// entonces dotenv ya corrio y el secreto existe.
let _llave = null;
function llave() {
  if (!_llave) _llave = crypto.createHash('sha256').update('wx-oauth-state|' + secreto()).digest();
  return _llave;
}
