...
# Reemplazar la funcion pintarProductoApp completa
start = c.find('function pintarProductoApp(')
end = c.find('function agregarApp(')

if start == -1 or end == -1 or end < start:
    print("ERROR: no se encontro el rango de pintarProductoApp")
else:
  ...
  if (imgs.length) {
    galeria = '<div style="position:relative;background:#fff">' +
      '<div id="prod-gal" style="display:flex;overflow-x:auto;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none">' +
  ...
  var descHTML = '';
  if (p.descripcion_es) {
    var parrafos = p.descripcion_es.split('\\n').filter(function(x){ return x.trim(); }).map(function(x){ return '<p style="margin-bottom:8px">' + x + '</p>'; }).join('');
    descHTML = '<div style="font-size:0.78rem;font-weight:800;color:#F0F4F8;margin:22px 0 8px">Descripción</div>' +
      '<div style="font-size:0.9rem;color:var(--gray);line-height:1.6">' + parrafos + '</div>';
  }
  ...
  if (p.specs && typeof p.specs === 'object' && Object.keys(p.specs).length) {
    ...
    specsHTML = '<div style="font-size:0.78rem;font-weight:800;color:#F0F4F8;margin:22px 0 8px">Especificaciones</div>' +
      '<table style="width:100%;border-collapse:collapse;font-size:0.84rem;border:1px solid var(--line);border-radius:10px;overflow:hidden"><tbody>' + rows + '</tbody></table>';
  }

  // ── Documentos ──
  var docsHTML = '';
  if (Array.isArray(p.archivos) && p.archivos.length) {
    docsHTML = '<div style="font-size:0.78rem;font-weight:800;color:#F0F4F8;margin:22px 0 8px">Documentos</div>' +
      p.archivos.map(function(f){
        return '<a href="' + f.url + '" target="_blank" style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--card);border:1px solid var(--line);border-radius:10px;margin-bottom:8px;text-decoration:none">' +
          ...
          '<div style="font-size:0.72rem;color:var(--gray2)">' + (f.tamano || 'Descargar') + '</div></div>' +
        '</a>';
      }).join('');
  }

  // ── Precio (con anterior tachado) ──
  var precioBloque;
  if (precioInfo) {
    precioBloque = '<div style="display:flex;align-items:baseline;gap:10px;margin-bottom:6px">' +
      '<div style="font-size:1.8rem;font-weight:800;color:var(--bl-l)">$' + parseFloat(precioInfo.precio).toFixed(2) + '</div>' +
      (p.precio_anterior ? '<div style="font-size:1rem;color:var(--gray2);text-decoration:line-through">$' + parseFloat(p.precio_anterior).toFixed(2) + '</div>' : '') +
      '</div>' +
      '<div style="font-size:0.74rem;color:var(--gray2);margin-bottom:8px">+ IVU 11.5% al pagar</div>';
  } else {
    precioBloque = '<div style="font-size:1rem;color:var(--gray);margin-bottom:10px">Precio no disponible</div>';
  }
  ...
  modal.innerHTML =
    ...
    '</div>' +
    '<div style="position:fixed;bottom:0;left:0;right:0;padding:14px 16px calc(14px + env(safe-area-inset-bottom));background:rgba(6,9,15,0.96);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border-top:1px solid var(--line)">' +
...
'''
    ...
    with open('/var/www/wifnix/app/index.html','w') as f:
        f.write(c)
    print("pintarProductoApp reconstruido (diseño + precio anterior + docs)")
